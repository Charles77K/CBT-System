...
import { useCreateCandidate, useFetchExams } from "../../../services/hooks";
            ...
            {examData.map((exam, index) => (
              <label key={index} className="flex items-center space-x-2">
                <input
                  type="checkbox"
                  value={exam._id}
                  checked={selectedExams.includes(exam._id)} // Controlled component
                  onChange={(e) => {
                    const newSelectedExams = e.target.checked
                      ? [...selectedExams, exam._id]
                      : selectedExams.filter((id: string) => id !== exam._id);
